      ...
      <div className="relative flex justify-center pb-4 pt-2">
        <ResponsiveContainer width={158} height={158}>
          <PieChart>
            ...
            <Tooltip
              formatter={(v: number, name: string) => [`${Math.round(v / total * 100)}%`, name]}
              contentStyle={{ background: 'var(--sp-surface)', border: '1px solid var(--sp-border)', borderRadius: 12, fontSize: 12 }}
            />
          </PieChart>
        </ResponsiveContainer>
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span className="tnum font-bold text-[28px] leading-none" style={{ fontFamily: 'var(--sp-display)' }}>
      ...
      <div className="flex flex-col gap-2">
        {data.map((d) => (
          <div key={d.label} className="flex items-center gap-2">
            ...
            <span className="t-small tnum text-sp-t2">{Math.round(d.value / total * 100)}%</span>
          </div>
        ))}
      </div>
    </>
