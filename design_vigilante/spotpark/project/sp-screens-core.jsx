  ...
  if (i === 0) return (
    <>
      ...
      <div className="rise-s" style={{ position:'absolute', left:24, right:24, bottom:54, background:'#fff', borderRadius:18, padding:13, display:'flex', gap:12, alignItems:'center', boxShadow:'var(--sh-pop)', animation:'sp-float 3.4s ease-in-out infinite' }}>
        ...
        <div style={{ textAlign:'right' }}><div className="t-h3 tnum" style={{ color:'var(--ink)' }}>$3.000</div><div className="t-micro" style={{ color:'var(--t2)' }}>/hora</div></div>
      </div>
    </>
  );
  if (i === 1) return (
  ...
  return (
    <div className="scr-scroll no-sb" style={{ position:'absolute', inset:0, background:'var(--ink)' }}>
      ...
      <div style={{ background:'var(--ink)', padding:'4px 24px 36px', display:'flex', flexDirection:'column', minHeight:'calc(100% - 330px)' }}>
        ...
        <button onClick={()=> reg ? setMode('login') : push('register')} style={{ background:'none', border:'none', cursor:'pointer', margin:'20px auto 0', fontSize:14 }}>
...
function FocusInput({ ic, ph, type='text', trailing, prefix, value, onChange }) {
  ...
  return (
    <div className={'input'+(f?' focus':'')}>
      ...
      <input type={type} placeholder={ph} value={value} onChange={onChange?(e=>onChange(e.target.value)):undefined}
...
/* ─────────── REGISTER (multi-step) ─────────── */
const REG_STEPS = [
  { key:'datos',    label:'Tus datos',    title:'Cuéntanos quién eres',     sub:'Usaremos esto para tus reservas y recibos.' },
  { key:'clave',    label:'Seguridad',    title:'Crea tu contraseña',        sub:'Mínimo 8 caracteres. Que sea difícil de adivinar.' },
  { key:'vehiculo', label:'Vehículo',     title:'Agrega tu primer vehículo', sub:'Para reservar más rápido. Puedes añadir más luego.' },
];
function Register() {
  const { replace, pop } = useNav();
  const [step, setStep] = uS(0);
  const [done, setDone] = uS(false);
  const [show, setShow] = uS(false);
  const [f, setF] = uS({ name:'', email:'', phone:'', pass:'', confirm:'', plate:'', vtype:'car', color:'#C6F24E' });
  const set = (k,v) => setF(p=>({ ...p, [k]:v }));

  // password strength
  const pass = f.pass;
  const strength = (pass.length>=8?1:0) + (/[A-Z]/.test(pass)?1:0) + (/[0-9]/.test(pass)?1:0) + (/[^A-Za-z0-9]/.test(pass)?1:0);
  const strLabel = ['Muy débil','Débil','Aceptable','Fuerte','Excelente'][strength];
  const strColor = ['#E5484D','#E5484D','#E0A211','#5E7F12','#5E7F12'][strength];

  const valid = [
    f.name.trim().length>2 && /\S+@\S+\.\S+/.test(f.email) && f.phone.replace(/\D/g,'').length>=10,
    pass.length>=8 && pass===f.confirm,
    f.plate.replace(/[^A-Z0-9]/g,'').length>=5,
  ];
  const colors = ['#C6F24E','#0F1115','#E5484D','#4C8DFF','#E8851E','#FFFFFF'];

  const next = () => { if (step<REG_STEPS.length-1) setStep(step+1); else setDone(true); };
  const back = () => { if (done) return; if (step>0) setStep(step-1); else pop(); };

  if (done) return (
    <div className="scr-scroll no-sb" style={{ position:'absolute', inset:0, background:'var(--ink)' }}>
      <Chrome onDark />
      <div style={{ minHeight:'100%', display:'flex', flexDirection:'column', alignItems:'center', padding:'92px 28px 40px', position:'relative' }}>
        <div style={{ position:'absolute', top:120, width:230, height:230 }}>
          {[0,1,2].map(k=>(<div key={k} style={{ position:'absolute', inset:0, borderRadius:'50%', border:'1.5px solid rgba(198,242,78,.32)', animation:'sp-ping 2.6s ease-out infinite', animationDelay:`${k*.6}s` }}/>))}
        </div>
        ...
        <h1 className="t-h1 rise-s" style={{ margin:'24px 0 8px', color:'#fff', textAlign:'center' }}>¡Bienvenido, {f.name.split(' ')[0]||'a SpotPark'}!</h1>
        <p className="t-body rise-s d1" style={{ color:'rgba(255,255,255,.6)', margin:0, textAlign:'center', maxWidth:280 }}>Tu cuenta está lista. Encuentra y reserva tu próxima plaza en segundos.</p>
        <div className="rise-s d2" style={{ width:'100%', marginTop:30 }}>
          <button className="btn btn-primary btn-block" onClick={()=>replace('home')}>Comenzar <Icon n="arrR" s={19} c="var(--ink)"/></button>
        </div>
      </div>
    </div>
  );

  const st = REG_STEPS[step];
  return (
    <div className="scr-scroll no-sb" style={{ position:'absolute', inset:0, background:'var(--ink)', display:'flex', flexDirection:'column' }}>
      <Chrome onDark />
      {/* header: back + progress */}
      <div style={{ padding:'58px 24px 0', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:14 }}>
          <RoundBtn glass onClick={back}><Icon n="chevL" s={21} c="#fff"/></RoundBtn>
          <div style={{ flex:1, display:'flex', gap:6 }}>
            {REG_STEPS.map((_,k)=>(
              <div key={k} style={{ flex:1, height:5, borderRadius:9, overflow:'hidden', background:'rgba(255,255,255,.12)' }}>
                <div style={{ height:'100%', borderRadius:9, background:'var(--blue)', transformOrigin:'left',
                  transform:`scaleX(${k<step?1:k===step?1:0})`, transition:'transform .4s cubic-bezier(.3,1,.4,1)' }}/>
              </div>
            ))}
          </div>
          <span className="t-micro tnum" style={{ color:'rgba(255,255,255,.5)' }}>{step+1}/{REG_STEPS.length}</span>
        </div>
      </div>

      <div className="scr-scroll no-sb" key={step} style={{ flex:1, padding:'26px 24px 20px' }}>
        <div className="t-micro upper rise-s" style={{ color:'var(--blue)', letterSpacing:'.14em' }}>{st.label}</div>
        <h1 className="t-h1 rise-s d1" style={{ color:'#fff', margin:'8px 0 6px' }}>{st.title}</h1>
        <p className="t-small rise-s d2" style={{ color:'rgba(255,255,255,.55)', margin:'0 0 26px' }}>{st.sub}</p>

        {step===0 && (
          <div className="fade" style={{ display:'flex', flexDirection:'column', gap:12 }}>
            <FocusInput ic="user" ph="Nombre completo" value={f.name} onChange={v=>set('name',v)}/>
            <FocusInput ic="mail" ph="Correo electrónico" value={f.email} onChange={v=>set('email',v)}/>
            <FocusInput ic="phone" ph="Teléfono" prefix="+57" value={f.phone} onChange={v=>set('phone',v.replace(/[^\d]/g,'').slice(0,10))}/>
          </div>
        )}

        {step===1 && (
          <div className="fade" style={{ display:'flex', flexDirection:'column', gap:12 }}>
            <FocusInput ic="lock" ph="Contraseña" type={show?'text':'password'} value={f.pass} onChange={v=>set('pass',v)}
              trailing={<button onClick={()=>setShow(s=>!s)} style={{ background:'none', border:'none', cursor:'pointer', display:'flex' }}><Icon n={show?'eyeOff':'eye'} s={20} c="var(--t3)"/></button>}/>
            {pass && (
              <div className="rise-s" style={{ display:'flex', alignItems:'center', gap:10 }}>
                <div style={{ flex:1, display:'flex', gap:4 }}>
                  {[0,1,2,3].map(k=>(<div key={k} style={{ flex:1, height:4, borderRadius:9, background: k<strength?strColor:'rgba(255,255,255,.12)', transition:'background .25s' }}/>))}
                </div>
                <span style={{ fontSize:12, fontWeight:600, color:strColor }}>{strLabel}</span>
              </div>
            )}
            <FocusInput ic="lock" ph="Confirmar contraseña" type="password" value={f.confirm} onChange={v=>set('confirm',v)}
              trailing={f.confirm ? <Icon n={f.confirm===pass?'checkC':'x'} s={20} c={f.confirm===pass?'#5E7F12':'var(--red)'}/> : null}/>
          </div>
        )}

        {step===2 && (
          <div className="fade" style={{ display:'flex', flexDirection:'column', gap:18 }}>
            <FocusInput ic="car" ph="Placa (ej. ABC123)" value={f.plate}
              onChange={v=>set('plate', v.toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,6))}/>
            <div>
              <div className="t-small" style={{ color:'rgba(255,255,255,.6)', marginBottom:10 }}>Tipo de vehículo</div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                {SP.vehicleTypes.map(v=>{
                  const on=f.vtype===v.type;
                  return (
                    <button key={v.type} onClick={()=>set('vtype',v.type)} style={{ height:60, borderRadius:14, cursor:'pointer', gap:9, padding:'0 14px',
                      background:on?'rgba(198,242,78,.14)':'rgba(255,255,255,.05)', border:on?'1.5px solid var(--blue)':'1px solid rgba(255,255,255,.1)',
                      color: on?'#fff':'rgba(255,255,255,.7)', display:'flex', alignItems:'center', fontSize:15, fontWeight:600, transition:'all .15s' }}>
                      <span style={{ fontSize:22 }}>{v.emoji}</span>{v.label}
                      {on && <Icon n="checkC" s={18} c="var(--blue)" style={{ marginLeft:'auto' }}/>}
                    </button>
                  );
                })}
              </div>
            ...
            <div>
              <div className="t-small" style={{ color:'rgba(255,255,255,.6)', marginBottom:10 }}>Color (opcional)</div>
              <div style={{ display:'flex', gap:12 }}>
                {colors.map(c=>(
                  <button key={c} onClick={()=>set('color',c)} style={{ width:36, height:36, borderRadius:'50%', cursor:'pointer', background:c,
                    border: c==='#FFFFFF'?'1px solid rgba(255,255,255,.3)':'none',
                    boxShadow: f.color===c?'0 0 0 2px var(--ink), 0 0 0 4px var(--blue)':'none', transition:'box-shadow .15s' }}/>
                ))}
              </div>
            </div>
          </div>
      ...
      <div style={{ padding:'12px 24px 32px', flexShrink:0 }}>
        {step===2 && <button onClick={next} style={{ display:'block', margin:'0 auto 14px', background:'none', border:'none', color:'rgba(255,255,255,.5)', fontSize:14, fontWeight:500, cursor:'pointer' }}>Omitir por ahora</button>}
        <button className="btn btn-primary btn-block" disabled={!valid[step]} style={{ opacity: valid[step]?1:.45 }} onClick={next}>
          {step<REG_STEPS.length-1 ? 'Continuar' : 'Crear cuenta'} <Icon n="arrR" s={19} c="var(--ink)"/>
        </button>
      </div>
    </div>
  );
...
Object.assign(window.SP_SCREENS, { splash:Splash, onboarding:Onboarding, auth:Auth, register:Register, home:Home, search:Search });
