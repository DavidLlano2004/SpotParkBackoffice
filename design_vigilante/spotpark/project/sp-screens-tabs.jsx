...
/* reservation lifecycle = lime/ink accent only (green/yellow/red reserved for availability) */
const STATUS_COLOR = { active:'var(--blue)', upcoming:'var(--lime-deep)', past:'var(--t4)', cancelled:'var(--t4)' };

/* small cohesive lifecycle pill */
function LifeBadge({ status }) {
  if (status==='active') return <span className="badge" style={{ background:'var(--ink)', color:'#fff' }}><span style={{ width:6, height:6, borderRadius:'50%', background:'var(--blue)', animation:'sp-pulse 1.6s infinite' }}/>En curso</span>;
  if (status==='upcoming') return <span className="badge" style={{ background:'var(--blue-bg)', color:'var(--blue-light)' }}>Próxima</span>;
  return <span className="badge" style={{ background:'#ECECE8', color:'var(--t2)' }}>Completada</span>;
}
        ...
        {list.map((t,i)=>{
          ...
          return (
            <button key={t.id} className="rise-s" style={{ animationDelay:`${i*.06}s`, width:'100%', textAlign:'left', cursor:'pointer',
              background:'var(--surface)', border: active?'1px solid var(--lime-deep)':'.5px solid var(--border-card)', borderRadius:18, padding:0, marginBottom:14, overflow:'hidden', display:'flex' }}
              ...
              <div style={{ display:'flex', gap:13, padding:14, flex:1, minWidth:0 }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ marginTop:7 }}><span className="tnum" style={{ background:'#ECECE8', color:'var(--t1)', fontSize:11, fontWeight:600, padding:'3px 8px', borderRadius:6, letterSpacing:'.02em' }}>{t.plate}</span></div>
                </div>
                <div style={{ textAlign:'right', display:'flex', flexDirection:'column', alignItems:'flex-end', gap:8 }}>
                  <span className="t-h3 tnum">{SP.COP(t.total)}</span>
                  <LifeBadge status={t.status}/>
                </div>
              </div>
            </button>
          );
    ...
    <div style={{ position:'absolute', inset:0, background:'var(--bg)', display:'flex', flexDirection:'column' }}>
      ...
      <div className="scr-scroll no-sb" style={{ padding:'20px 20px 30px' }}>
        ...
        <div className="card" style={{ padding:16, marginTop:24, background:'var(--surface)' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between' }}><span className="t-h3">Total</span><span className="t-h3 tnum" style={{ color:'var(--ink)' }}>{SP.COP(t.total)}</span></div>
        </div>
        ...
        {saved.map((s,i)=>(
          <button key={s.id} className="rise-s" style={{ animationDelay:`${i*.06}s`, width:'100%', textAlign:'left', cursor:'pointer',
            ...
            <div style={{ padding:'12px 14px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              ...
              <div style={{ textAlign:'right' }}><span className="t-h3 tnum" style={{ color:'var(--ink)' }}>{SP.COP(s.price)}</span><div className="t-micro" style={{ color:'var(--t2)' }}>/hora</div></div>
            </div>
          </button>
        ))}
      </div>
    ...
    <div style={{ position:'absolute', inset:0, background:'var(--bg)', display:'flex', flexDirection:'column' }}>
      ...
      <div className="scr-scroll no-sb" style={{ padding:'58px 16px 110px' }}>
        ...
        <div className="rise-s" style={{ borderRadius:'var(--r-xl)', padding:22, background:'var(--ink)', boxShadow:'var(--sh-ink)', position:'relative', overflow:'hidden' }}>
          ...
          <button onClick={()=>push('puntos')} style={{ display:'flex', alignItems:'center', gap:8, marginTop:14, position:'relative', background:'rgba(255,255,255,.08)', border:'none', borderRadius:12, padding:'10px 12px', width:'100%', cursor:'pointer' }}>
            <Icon n="bolt" s={16} c="var(--lime)"/>
            <span className="tnum" style={{ color:'var(--lime)', fontWeight:700, fontSize:15 }}>{SP.loyalty.points.toLocaleString('es-CO')}</span>
            <span style={{ color:'rgba(255,255,255,.7)', fontSize:13 }}>SpotPoints</span>
            <span className="badge" style={{ background:'rgba(255,255,255,.14)', color:'#fff', marginLeft:4 }}>{SP.loyalty.level}</span>
            <Icon n="chevR" s={16} c="rgba(255,255,255,.5)" style={{ marginLeft:'auto' }}/>
          </button>
        ...
        {/* quick access: gastos + AI */}
        <div style={{ display:'flex', gap:12, marginTop:14 }}>
          <button className="rise-s d1" onClick={()=>push('gastos')} style={{ flex:1, textAlign:'left', cursor:'pointer', background:'var(--surface)', border:'1px solid var(--border-card)', borderRadius:18, padding:16, boxShadow:'var(--sh-card)' }}>
            <div style={{ width:40, height:40, borderRadius:12, background:'var(--elevated)', display:'flex', alignItems:'center', justifyContent:'center' }}><Icon n="card" s={20} c="var(--ink)"/></div>
            <div style={{ fontSize:15, fontWeight:500, marginTop:10 }}>Mis gastos</div>
            <div className="t-micro" style={{ color:'var(--t2)' }}>{SP.COP(SP.spending.total)} este mes</div>
          </button>
          <button className="rise-s d2" onClick={()=>push('puntos')} style={{ flex:1, textAlign:'left', cursor:'pointer', background:'var(--surface)', border:'1px solid var(--border-card)', borderRadius:18, padding:16, boxShadow:'var(--sh-card)' }}>
            <div style={{ width:40, height:40, borderRadius:12, background:'var(--blue-tint)', display:'flex', alignItems:'center', justifyContent:'center' }}><Icon n="bolt" s={20} c="var(--lime-deep)"/></div>
            <div style={{ fontSize:15, fontWeight:500, marginTop:10 }}>SpotPoints</div>
            <div className="t-micro" style={{ color:'var(--t2)' }}>{SP.loyalty.points.toLocaleString('es-CO')} · {SP.loyalty.level}</div>
          </button>
        </div>
        ...
        {/* F2/F6: comunidad + eventos */}
        <div style={{ display:'flex', gap:12, marginTop:14 }}>
          <button className="rise-s d1" onClick={()=>push('red')} style={{ flex:1, textAlign:'left', cursor:'pointer', background:'var(--surface)', border:'1px solid var(--border-card)', borderRadius:18, padding:16, boxShadow:'var(--sh-card)' }}>
            <div style={{ width:40, height:40, borderRadius:12, background:'var(--blue-tint)', display:'flex', alignItems:'center', justifyContent:'center' }}><Icon n="user" s={20} c="var(--blue-light)"/></div>
            <div style={{ fontSize:15, fontWeight:500, marginTop:10 }}>Tu red</div>
            <div className="t-micro" style={{ color:'var(--t2)' }}>{SP.social.networkCount} contactos</div>
          </button>
          <button className="rise-s d2" onClick={()=>push('eventos')} style={{ flex:1, textAlign:'left', cursor:'pointer', background:'var(--surface)', border:'1px solid var(--border-card)', borderRadius:18, padding:16, boxShadow:'var(--sh-card)' }}>
            <div style={{ width:40, height:40, borderRadius:12, background:'var(--elevated)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:18 }}>🎵</div>
            <div style={{ fontSize:15, fontWeight:500, marginTop:10 }}>Eventos</div>
            <div className="t-micro" style={{ color:'var(--t2)' }}>{SP.events.length} cerca de ti</div>
          </button>
        </div>
