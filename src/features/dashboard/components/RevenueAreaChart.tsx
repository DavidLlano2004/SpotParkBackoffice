...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div
      ...
      style={{ background: 'var(--sp-surface)', border: '1px solid var(--sp-border)', boxShadow: 'var(--sp-sh-pop)' }}
    >
  ...
  return (
    <ResponsiveContainer width="100%" height={206}>
      <AreaChart data={data} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
        ...
        <XAxis dataKey="label" tick={{ fontSize: 11, fill: 'var(--sp-t3)', fontFamily: 'var(--sp-font)' }} axisLine={false} tickLine={false} />
        <YAxis tickFormatter={formatCOPk} tick={{ fontSize: 11, fill: 'var(--sp-t3)', fontFamily: 'var(--sp-font)' }} axisLine={false} tickLine={false} />
        <Tooltip content={<CustomTooltip />} cursor={{ stroke: 'var(--sp-border)', strokeWidth: 1 }} />
        <Area
          type="monotone" dataKey="value"
          stroke="var(--sp-lime-deep)" strokeWidth={2}
          fill="url(#sp-area)"
          dot={false} activeDot={{ r: 4, fill: 'var(--sp-lime-deep)', stroke: 'var(--sp-surface)', strokeWidth: 2 }}
